...
<div id="buddypress">
	...
	<form action="<?php bp_group_creation_form_action(); ?>" method="post" id="create-group-form" class="standard-form" enctype="multipart/form-data">
		...
		<div id="template-notices" aria-atomic="true">
			<?php
		...
		<div class="item-body" id="group-create-body">
			...
			<?php if ( bp_is_group_creation_step( 'group-details' ) ) : ?>
				...
				<h2 class="bp-screen-reader-text"><?php
					...
					esc_html_e( 'Committee Details', 'flocks' );
				?></h2>
				...
				<div>
					<label for="group-name"><?php esc_html_e( 'Committee Name (required)', 'flocks' ); ?></label>
					<input type="text" name="group-name" id="group-name" aria-required="true" value="<?php bp_new_group_name(); ?>" />
				</div>

				<div>
					<label for="group-desc"><?php esc_html_e( 'Short Description (required)', 'flocks' ); ?></label>
			...
			<?php if ( bp_is_group_creation_step( 'group-settings' ) ) : ?>
				...
				<h2 class="bp-screen-reader-text"><?php
					/* translators: accessibility text */
					esc_html_e( 'Committee Settings', 'flocks' );
				?></h2>
				...
				<input type="hidden" name="group-status" id="group-status-private" value="private" checked="checked" aria-describedby="private-group-description" /> 
				...
				<?php if ( $group_types = bp_groups_get_group_types( array( 'show_in_create_screen' => true ), 'objects' ) ): ?>
					...
					<fieldset class="group-create-types">
						<legend><?php esc_html_e( 'Group Types', 'flocks' ); ?></legend>

						<p><?php esc_html_e( 'Select the types this group should be a part of.', 'flocks' ); ?></p>
								...
								<label for="<?php printf( 'group-type-%s', $type->name ); ?>"><input type="checkbox" name="group-types[]" id="<?php printf( 'group-type-%s', $type->name ); ?>" value="<?php echo esc_attr( $type->name ); ?>" <?php checked( true, ! empty( $type->create_screen_checked ) ); ?> /> <?php echo esc_html( $type->labels['name'] ); ?>
									<?php
										if ( ! empty( $type->description ) ) {
											...
											printf( esc_html__( '&ndash; %s', 'flocks' ), '<span class="bp-group-type-desc">' . esc_html( $type->description ) . '</span>' );
										}
									?>
								</label>
							</div>
				...
				<fieldset class="group-create-invitations">
					...
					<legend><?php esc_html_e( 'Invitations', 'flocks' ); ?></legend>

					<p><?php esc_html_e( 'Which members of this committee are allowed to invite others?', 'flocks' ); ?></p>

					<div class="radio">

						<label for="group-invite-status-members"><input type="radio" name="group-invite-status" id="group-invite-status-members" value="members"<?php bp_group_show_invite_status_setting( 'members' ); ?> /> <?php esc_html_e( 'All members', 'flocks' ); ?></label>

						<label for="group-invite-status-mods"><input type="radio" name="group-invite-status" id="group-invite-status-mods" value="mods"<?php bp_group_show_invite_status_setting( 'mods' ); ?> checked="checked" /> <?php esc_html_e( 'Group admins and mods only', 'flocks' ); ?></label>

						<label for="group-invite-status-admins"><input type="radio" name="group-invite-status" id="group-invite-status-admins" value="admins"<?php bp_group_show_invite_status_setting( 'admins' ); ?> /> <?php esc_html_e( 'Group admins only', 'flocks' ); ?></label>
				...
				<?php if ( bp_is_active( 'forums' ) ) : ?>
					...
					<h4><?php esc_html_e( 'Group Forums', 'flocks' ); ?></h4>

					<?php if ( bp_forums_is_installed_correctly() ) : ?>

						<p><?php esc_html_e( 'Should this group have a forum?', 'flocks' ); ?></p>

						<div class="checkbox">
							<label for="group-show-forum"><input type="checkbox" name="group-show-forum" id="group-show-forum" value="1"<?php checked( bp_get_new_group_enable_forum(), true, true ); ?> /> <?php esc_html_e( 'Enable discussion forum', 'flocks' ); ?></label>
						</div>
					<?php elseif ( is_super_admin() ) : ?>

						<p><?php printf( esc_html__( '<strong>Attention Site Admin:</strong> Group forums require the <a href="%s">correct setup and configuration</a> of a bbPress installation.', 'flocks' ), bp_core_do_network_admin() ? network_admin_url( 'settings.php?page=bb-forums-setup' ) :  admin_url( 'admin.php?page=bb-forums-setup' ) ); ?></p>
			...
			<?php if ( bp_is_group_creation_step( 'group-avatar' ) ) : ?>
				...
				<h2 class="bp-screen-reader-text"><?php
					/* translators: accessibility text */
					esc_html_e( 'Group Avatar', 'flocks' );
				?></h2>
				...
				<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
					...
					<div class="main-column">
						...
						<p><?php esc_html_e( 'To skip the group profile photo upload process, hit the "Next Step" button.', 'flocks' ); ?></p>
					</div><!-- .main-column -->
				...
				<?php if ( 'crop-image' == bp_get_avatar_admin_step() ) : ?>
					...
					<h4><?php esc_html_e( 'Crop Group Profile Photo', 'flocks' ); ?></h4>
			...
			<?php if ( bp_is_group_creation_step( 'group-cover-image' ) ) : ?>
				...
				<p><?php esc_html_e( 'The Cover Image will be used to customize the header of your group.', 'flocks' ); ?></p>
			...
			<?php if ( bp_is_group_creation_step( 'group-invites' ) ) : ?>
				...
				<h2 class="bp-screen-reader-text"><?php
					/* translators: accessibility text */
					esc_html_e( 'Group Invites', 'flocks' );
				?></h2>
				...
				<?php if ( bp_is_active( 'friends' ) && bp_get_total_friend_count( bp_loggedin_user_id() ) ) : ?>
					...
					<div class="main-column">
						...
						<div id="message" class="info">
							<p><?php esc_html_e('Select people to invite from your friends list.', 'flocks' ); ?></p>
						</div>
						...
						<?php if ( bp_group_has_invites() ) : ?>
							...
							<?php while ( bp_group_invites() ) : bp_group_the_invite(); ?>
								...
								<li id="<?php bp_group_invite_item_id(); ?>">
									...
									<div class="action">
										<a class="remove" href="<?php bp_group_invite_user_remove_invite_url(); ?>" id="<?php bp_group_invite_item_id(); ?>"><?php esc_html_e( 'Remove Invite', 'flocks' ); ?></a>
									</div>
								</li>
				...
				<?php else : ?>
					...
					<div id="message" class="info">
						<p><?php esc_html_e( 'Once you have built up friend connections you will be able to invite others to your group.', 'flocks' ); ?></p>
					</div>
			...
			<?php if ( 'crop-image' != bp_get_avatar_admin_step() ) : ?>
				...
				<div class="submit" id="previous-next">
					...
					<?php if ( !bp_is_first_group_creation_step() ) : ?>
						...
						<input type="button" value="<?php esc_attr_e( 'Back to Previous Step', 'flocks' ); ?>" id="group-creation-previous" name="previous" onclick="location.href='<?php bp_group_creation_previous_link(); ?>'" />
					...
					<?php if ( !bp_is_last_group_creation_step() && !bp_is_first_group_creation_step() ) : ?>
						...
						<input type="submit" value="<?php esc_attr_e( 'Next Step', 'flocks' ); ?>" id="group-creation-next" name="save" />
					...
					<?php if ( bp_is_first_group_creation_step() ) : ?>
						...
						<input type="submit" value="<?php esc_attr_e( 'Create Committee and Continue', 'flocks' ); ?>" id="group-creation-create" name="save" />
					...
					<?php if ( bp_is_last_group_creation_step() ) : ?>
						...
						<input type="submit" value="<?php esc_attr_e( 'Finish', 'flocks' ); ?>" id="group-creation-finish" name="save" />
