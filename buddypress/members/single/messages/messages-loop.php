			...
			<thead>
				<tr>
					<th scope="col" class="thread-checkbox bulk-select-all">
						...
						<?php
						/**
						* Remove message select all checkbox label.
						*
						* By Jasper
						* @since 1.0
						*/
					...
					<th scope="col" class="thread-from"><?php esc_html_e( 'From', 'flocks' ); ?></th>
					<th scope="col" class="thread-info"><?php esc_html_e( 'Subject', 'flocks' ); ?></th>
					...
					<?php if ( bp_is_active( 'messages', 'star' ) ) : ?>
						<th scope="col" class="thread-star"><span class="message-action-star"><span class="icon"></span> <span class="screen-reader-text"><?php
							...
							esc_html_e( 'Star', 'flocks' );
						?></span></span></th>
					<?php endif; ?>

					<th scope="col" class="thread-options"><?php esc_html_e( 'Actions', 'flocks' ); ?></th>
				</tr>
			</thead>
				...
				<?php while ( bp_message_threads() ) : bp_message_thread(); ?>
					...
					<tr id="m-<?php bp_message_thread_id(); ?>" class="<?php bp_message_css_class(); ?><?php if ( bp_message_thread_has_unread() ) : ?> unread<?php else: ?> read<?php endif; ?>">
						<td class="bulk-select-check">
							...
							<?php
							/**
							* Remove message select checkbox label.
							*
							* By Jasper
							* @since 1.0
							*/
						...
						<?php if ( 'sentbox' != bp_current_action() ) : ?>
							<td class="thread-from">
								<?php bp_message_thread_avatar( array( 'width' => 125, 'height' => 125 ) ); ?>

								<?php
								/**
								* Remove message from label.
								*
								* By Jasper
								* @since 1.0
								*/
						...
						<?php else: ?>
							<td class="thread-from">
								<?php bp_message_thread_avatar( array( 'width' => 125, 'height' => 125 ) ); ?>
								<span class="to"><?php esc_html_e( 'To:', 'flocks' ); ?></span> <?php bp_message_thread_to(); ?>
						...
						<td class="thread-info">
							<h5 class="mg-top-15">
								<a href="<?php bp_message_thread_view_link(); ?>" title="<?php esc_attr_e( "View Message", 'flocks' ); ?>">
									<?php bp_message_thread_subject(); ?>
								</a>
							</h5>
							<p class="thread-excerpt"><?php bp_message_thread_excerpt(); ?></p>
							<p><span class="activity"><small><?php bp_message_thread_last_post_date(); ?></small></span></p>
						...
						<td class="thread-options">
							<?php if ( bp_message_thread_has_unread() ) : ?>
								<a class="read" href="<?php bp_the_message_thread_mark_read_url();?>"><?php esc_html_e( 'Read', 'flocks' ); ?></a>
							<?php else : ?>
								<a class="unread" href="<?php bp_the_message_thread_mark_unread_url();?>"><?php esc_html_e( 'Unread', 'flocks' ); ?></a>
							<?php endif; ?>
							 |
							<a class="delete" href="<?php bp_message_thread_delete_link(); ?>"><?php esc_html_e( 'Delete', 'flocks' ); ?></a>
...
<?php else: ?>
	...
	<div id="message" class="info">
		<p><?php esc_html_e( 'Sorry, no messages were found.', 'flocks' ); ?></p>
	</div>
